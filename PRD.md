...
- **Detalhamento do Serviço**: Modal otimizado que consolida todas as informações (Orçamento, Cliente, Valor Orçado, Custos, Saldo) sem necessidade de rolagem excessiva.
...
- **Controle de Pagamentos**: Tela dedicada para registrar e acompanhar o que já foi recebido vs. o que está pendente por serviço.
...
- **Análise de Custos por Categoria**: Visão detalhada do peso de cada custo (ex: peças, MO) dentro de sua categoria (%) e no negócio como um todo (% geral).
- **Top Clientes**: Ranking dos clientes que mais geram receita e margem de lucro.
- **Situação Financeira**: Resumo rápido de serviços totais, valores recebidos e a receber.

### 3.4 Integrações e Configurações
- **Evolution API**: Integração para comunicações automatizadas via WhatsApp (configuração de credenciais via interface dedicada).
- **Segurança**: Módulo de verificação de segurança do App integrado nas configurações.
- **Exportação**: Geração de relatórios em Excel para contabilidade ou backups.

## 4. REQUISITOS NÃO FUNCIONAIS
- **Interface Responsiva**: Otimizada para uso em desktop e dispositivos móveis.
- **Segurança**: Proteção de rotas (`ProtectedRoute`) e auditoria de segurança periódica.
- **Performance**: Dashboards carregados via React Query para garantir fluidez nos dados e UI reativa.

## 5. ARQUITETURA TÉCNICA
- **Frontend**: React + Vite + Tailwind CSS + Shadcn UI.
- **Estado e Dados**: Context API (filtros) + TanStack Query.
